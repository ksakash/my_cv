...
private:
  ...
  bool successTorpedo, sideCenter, IP_stopped, heightGoal;
  float present_depth, present_Y_coord;
  int min_pwm;

public:
  TaskTorpedoInnerClass(std::string name, std::string node, std::string node1, std::string node2, std::string node3)
    : torpedo_server_(nh_, name, boost::bind(&TaskTorpedoInnerClass::analysisCB, this, _1), false)
    , action_name_(name)
    , ForwardClient_(node)
    ...
    , TurnClient_(node3)
  {
    ...
    torpedo_server_.registerPreemptCallback(boost::bind(&TaskTorpedoInnerClass::preemptCB, this));
    ...
    sub_ip_ =
        nh_.subscribe<std_msgs::Float64MultiArray>("/varun/ip/torpedo", 1000, &TaskTorpedoInnerClass::torpedoNavigation, this);
    yaw_sub_ = nh_.subscribe<std_msgs::Float64>("/varun/sensors/imu/yaw", 1000, &TaskTorpedoInnerClass::yawCB, this);
    depth_sub_ = nh_.subscribe<std_msgs::Float64>("/varun/sensors/pressure_sensor/depth",
        1000, &TaskTorpedoInnerClass::depthCB, this);
    torpedo_server_.start();
  }

  ~TaskTorpedoInnerClass(void)
  {
  }
  ...
  void torpedoNavigation(std_msgs::Float64MultiArray array)
  {
    ...
    // if distance is -1 to -4 then torpedo is out of frame and the motion library will assume the last data.
    else if (data_distance_.data == -5 && sideCenter)
    {
      IP_stopped = true;
      stopTorpedoDetection();
  ...
  void analysisCB(const task_commons::torpedoGoalConstPtr goal)
  {
    ...
    TaskTorpedoInnerClass::startTorpedoDetection();
    ...
    boost::thread spin_thread_sideward_camera(&TaskTorpedoInnerClass::spinThreadSidewardCamera, this);

    while (goal->order && successTorpedo)
    {
      if (torpedo_server_.isPreemptRequested() || !ros::ok())
      {
        ROS_INFO("%s: Preempted", action_name_.c_str());
    ...
    ROS_INFO("%s: Bot is moving forward to hit the torpedo", action_name_.c_str());
    ...
    result_.MotionCompleted = successTorpedo;
    ROS_INFO("%s: Succeeded", action_name_.c_str());
    // set the action state to succeeded
    torpedo_server_.setSucceeded(result_);
  }

  void startTorpedoDetection()
  {
    std_msgs::Bool msg;
    msg.data = false;
...
int main(int argc, char **argv)
{
  ...
  TaskTorpedoInnerClass taskTorpedoObject(ros::this_node::getName(), "forward", "sideward", "upward", "turningXY");
  ros::spin();
  return 0;
}
