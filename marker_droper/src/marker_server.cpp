...
#include <task_commons/markerAction.h>
...
private:
  ...
  ros::Publisher switch_marker_detection;
  ...
  bool successMarker, sideCenter, IP_stopped, heightGoal;
  float present_depth, present_Y_coord;
  int min_pwm;

public:
  TaskMarkerInnerClass(std::string name, std::string node, std::string node1, std::string node2, std::string node3)
    ...
    , TurnClient_(node3)
  {
    ...
    sub_ip_ =
        nh_.subscribe<std_msgs::Float64MultiArray>("/varun/ip/marker", 1000, &TaskBuoyInnerClass::buoyNavigation, this);
